...
void MeshRenderer::draw_mesh(Mat4 model,
		...
		Color4 tint_color, ShaderMode mode) {
	...
	if(img_itr == batched_images.end()) {
		...
		draw_commands.push_back({model, mesh, submesh, texture, tint_color, image_count, (unsigned int)mode});
		image_count++;
	} else {
		draw_commands.push_back({model, mesh, submesh, texture, tint_color, image_count, mode});
	}
}
...
void MeshRenderer::render(u32 deltatime) {
    ...
    for(auto [entity, trans, mesh] : view.each()) {
		draw_mesh(trans.get_transform(), mesh.mesh.get(), mesh.sub_mesh, mesh.texture.get());
    }
